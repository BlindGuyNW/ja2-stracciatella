...
namespace
{
	...
	const Entry kTable[] =
	{
		...
		{ "stats",     &Cmd_Stats,      "stats [name] — level, attributes, skills, traits"       },
		...
		{ "move",       &Cmd_Move,       "move <target> [run] — walk (or run) to a target; opens doors" },
		...
		{ "attach",     &Cmd_Attach,     "attach <slot> detonator|remotedet — wire a detonator to a bomb" },
		{ "arm",        &Cmd_Arm,        "arm <slot> [turns <N> | freq <F> | pressure] — arm a wired bomb" },
		...
		{ "stash",      &Cmd_Stash,      "stash [<sector>] [hidden] — items in a sector's loot pile, one row per pile with @ offset; 'hidden' shows items the player hasn't spotted yet; 'stash take <pile> [merc]' grabs everything at an addressed tile (use 'nearby items' for the i<N> tags)" },
		...
		{ "battle",    &Cmd_Battle,     "battle | battle go | battle auto | battle retreat — pre-battle sector-arrival dialog" },
